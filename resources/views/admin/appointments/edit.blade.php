...
<div class="mb-6 md:mb-8">
    <a href="{{ route('admin.appointments.show', $appointment) }}" class="text-primary-600 hover:text-primary-700 text-sm font-medium inline-flex items-center gap-1 mb-2">
        <i class="fas fa-arrow-left"></i> Back to Appointment
    </a>
    <h1 class="text-xl md:text-2xl font-bold text-gray-900">Edit Appointment</h1>
    <p class="text-gray-600 text-sm">Update appointment details for {{ $appointment->patient_name }}</p>
</div>
            ...
            <div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
                ...
                <div class="space-y-4">
                    <div>
                        ...
                        <div class="relative" id="patient-search-wrapper">
                            ...
                            <div id="patient_dropdown" class="absolute z-20 w-full mt-1 bg-white border border-gray-200 rounded-lg shadow-lg max-h-60 overflow-y-auto hidden">
                                @forelse($patients as $patient)
                                    <div class="patient-option px-4 py-3 hover:bg-primary-50 cursor-pointer border-b border-gray-100 last:border-0 transition {{ $patient->id === $appointment->patient_id ? 'bg-primary-50' : '' }}"
                                        data-id="{{ $patient->id }}"
                        ...
                        <div id="selected-patient-display" class="mt-2 {{ $appointment->patient_id ? '' : 'hidden' }}">
            ...
            <div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="date" name="date_of_birth" id="date_of_birth" value="{{ old('date_of_birth', $appointment->patient?->date_of_birth?->format('Y-m-d')) }}"
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="address" id="address" rows="2"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition @error('address') border-red-500 @enderror">{{ old('address', $appointment->address) }}</textarea>
            ...
            <div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="text" name="department" id="department" value="{{ old('department', $appointment->department) }}"
                    ...
                    <div>
                        ...
                        <input type="date" name="preferred_date" id="preferred_date" value="{{ old('preferred_date', $appointment->preferred_date->format('Y-m-d')) }}" required
                    ...
                    <div>
                        ...
                        <input type="time" name="preferred_time" id="preferred_time" value="{{ old('preferred_time', $appointment->preferred_time) }}" required
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition @error('status') border-red-500 @enderror">
                    ...
                    <div class="sm:col-span-2">
                        ...
                        <textarea name="message" id="message" rows="3"
                            class="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition @error('message') border-red-500 @enderror">{{ old('message', $appointment->message) }}</textarea>
        ...
        <div class="space-y-4 md:space-y-6">
            <div class="bg-white rounded-xl shadow-sm p-4 md:p-6">
                ...
                <div class="space-y-2">
                    ...
                    <a href="{{ route('admin.appointments.show', $appointment) }}" class="block w-full text-center bg-gray-200 text-gray-700 px-4 py-2.5 rounded-lg font-medium hover:bg-gray-300 transition text-sm">
                        Cancel
                    </a>
                    <form action="{{ route('admin.appointments.destroy', $appointment) }}" method="POST" onsubmit="event.preventDefault(); swalConfirm(this, 'Delete Appointment?', 'This action cannot be undone.')">
                        @csrf
                        @method('DELETE')
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        @if($appointment->patient_id)
            (function() {
                const dropdown = document.getElementById('patient_dropdown');
                const selected = dropdown.querySelector('[data-id="{{ $appointment->patient_id }}"]');
