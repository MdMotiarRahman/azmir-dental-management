...
<div class="mb-6 md:mb-8">
    ...
    <h1 class="text-xl md:text-2xl font-bold text-gray-900">Edit Service: {{ $service->title }}</h1>
</div>
    ...
    <form action="{{ route('admin.services.update', $service) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
            ...
            <div>
                ...
                @if($service->image)
                    <div class="mb-2">
                        <img src="{{ $service->image_url }}" alt="{{ $service->title }}" class="w-16 h-16 object-cover rounded-lg">
                    </div>
                @endif
            ...
            <div class="md:col-span-2">
                ...
                <textarea name="description" id="description" rows="3"
                    class="w-full border border-gray-300 rounded-lg px-3 md:px-4 py-2.5 text-sm focus:ring-2 focus:ring-primary-500 focus:border-primary-500 transition @error('description') border-red-500 @enderror">{{ old('description', $service->description) }}</textarea>
